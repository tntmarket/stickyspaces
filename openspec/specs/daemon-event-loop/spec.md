...
### Requirement: DEL-NFR-2 Main thread logic readable in a single function

The system SHALL make the daemon's main thread logic understandable by reading a single function, because the original `while true { Task.sleep }` workaround, the `NSApplication.run()` deadlock, and future event-pump hacks all stem from the same subtle concurrency interaction, and the next developer MUST be able to reason about it clearly.

#### Scenario: Entry point is self-contained

- **WHEN** a developer reads the `main()` function in `EntryPoint.swift`
- **THEN** the daemon mode path (Task + app.run) and CLI mode path (Task + CFRunLoopRun) are each visible as a linear sequence of < 20 lines with no indirection to helper run-loop functions

### Requirement: DEL-C-1 Single executable for CLI and daemon modes

The system SHALL remain a single executable serving both CLI and daemon modes via the `--daemon` flag, because the parent spec (C-4) defines `--daemon` as an internal mechanism and two separate binaries would complicate the auto-launch path and distribution.

#### Scenario: Daemon launched via --daemon flag

- **WHEN** the binary is invoked with `--daemon`
- **THEN** it starts in daemon mode with NSApplication.run() event loop

#### Scenario: CLI commands run without --daemon flag

- **WHEN** the binary is invoked without `--daemon` (e.g., `stickyspaces list`)
- **THEN** it runs as a CLI client, completes the command, and exits cleanly

### Requirement: DEL-C-2 Existing tests pass without modification

The system SHALL preserve compatibility with existing CLI integration tests and IPC integration tests without requiring test modifications, because these tests validate the parent spec's requirements and have no dependency on how the daemon's main thread is structured.

#### Scenario: CLI integration tests remain green

- **WHEN** the full test suite is run after the entry point refactor
- **THEN** all existing CLIClientModeTests pass without modification

#### Scenario: IPC round-trip tests remain green
