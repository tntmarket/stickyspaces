...
### Requirement: D-ZO-1 Snapshot-first overview contract
The system MUST build a full CanvasSnapshot before presentation and treat it as the single render contract, because this keeps rendering deterministic and testable and avoids presenter-side hidden data pulls.

#### Scenario: Snapshot is built before presentation
- **WHEN** the zoom-out overview is triggered
- **THEN** a complete CanvasSnapshot is constructed before any presentation or animation begins

#### Scenario: Presenter renders only from snapshot
- **WHEN** the Map Overview is displayed
- **THEN** the presenter renders exclusively from the provided CanvasSnapshot without making additional data queries

### Requirement: D-ZO-2 Layout resolution before every snapshot
The system MUST run layout resolution against live topology before every snapshot, without mutating persistent user-visible state during overview entry, because this guarantees new workspaces appear predictably while preserving the read-only overview contract.

#### Scenario: New workspace gets deterministic default position
- **WHEN** a new workspace is created and the user triggers zoom-out
- **THEN** the new workspace appears at a deterministic default position derived by layout resolution

#### Scenario: Layout resolution does not mutate store
- **WHEN** layout resolution runs during snapshot construction
- **THEN** the StickyStore state is not mutated by the resolution process

### Requirement: D-ZO-3 Screenshot-thumbnail rendering contract
The system MUST render each workspace region from screenshot-thumbnail data that reflects the real desktop composition at zoom-out time, because this avoids synthetic reconstructions drifting from what the user actually sees and keeps the overview visually trustworthy.

#### Scenario: Thumbnail reflects real desktop at zoom time
- **WHEN** a workspace has windows and a wallpaper visible at zoom-out time
- **THEN** the screenshot thumbnail captures that real composition, not a synthetic reconstruction

#### Scenario: Thumbnail metadata is stable and non-empty
- **WHEN** zoom-out snapshots are taken repeatedly without changes
- **THEN** workspace screenshot-thumbnail metadata remains stable and non-empty across invocations

### Requirement: D-ZO-4 Two-step presentation API
The system MUST keep prepare and animate steps separable in the automation API, because this enables deterministic screenshot checkpoints and transition evidence capture between the two phases.

#### Scenario: Prepare without animate yields inspectable state
- **WHEN** an automation agent calls prepare without calling animate
- **THEN** the presentation state is ready for inspection and screenshot capture without any animation having occurred

#### Scenario: Animate after prepare triggers transition
- **WHEN** an automation agent calls animate after a successful prepare
- **THEN** the zoom-out transition animation plays from the prepared state to the final overview
